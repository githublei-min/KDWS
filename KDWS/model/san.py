...
class SAN(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        # middle_features = [x for x in args.features if x != ',']
        # self.middle_features = [args.n_resgroups - int(args.n_resgroups/len(middle_features))*int(x) - 1 for x in middle_features ]
        # print('SAN USED MIDDLE FEATURE MAPS OF ', self.middle_features)
    ...
    def forward(self, x):
        ...
        for i,l in enumerate(self.RG):
            ...
            # if i in self.middle_features:
            #     feature_maps.append(xx)
    ...
    def load_state_dict_teacher(self, state_dict, strict=False):
    ...
    def load_state_dict_student(self, state_dict, strict=False):
        own_state = self.state_dict()
        for name, param in state_dict.items():
            if name in own_state:
                if isinstance(param, nn.Parameter):
